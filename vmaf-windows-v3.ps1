...
if ($platform -eq "WINDOWS") {
    ...
    # VMAFAV1SOFTWAREIDENTIFIER IS JUST USE FOR PRELIMINARY LIKE ADDING PPA, AFTER THIS IS JUST UPDATE THRU APT
    sudo apt update; sudo apt upgrade -y; sudo apt install ffmpeg gcc cmake yasm clang-11 -y; rustup update;
    
    Write-Host "`nPREREQUISITES IS COMPLETE!"; Pause;
    
    Set-Location ~\;                                                    # CD TO HOME
    cargo install ab-av1;                                               # INSTALL AB-AV1 VIA CARGO
